...
Overview
You will run a three agentgenerative agent simulation — three LLM-powered characters living and working in a café — and investigate how small changes to their personality descriptions and memory architecture produce large, sometimes surprising, changes in behaviour.
...
A completed answer template (answer_template.md) with your analysis, log excerpts, plots, and written reflections.
Your modified files (scratch files, prompt edits).
Your analysis script (the completed analysis_starter.py or your own).


Step 0: Setup
0.1 Clone the repository
git clone -b course-exercises https://github.com/DorotaMoravcikova/generative_agents.git

cd generative_agents
0.2 Install Go
Download from go.dev/dl. Verify with:

go version

You need Go 1.24.3 or later. This is only needed to run the simulation server — you will not write or read any Go code.
...
Where
Why you need it
Dolores's personality
Look for scratch.txt under Dolores Abernathy's persona folder
You will edit this in Exercises 1 and 3
Maeve's personality
Same structure, Maeve Millay folder
Control agent, for comparison
Bernard's personality
Same structure, Bernard Lowe folder
You will edit this in Exercise 1
Simulation logs
Check simulation/logs/ after running
You will analyse these
Planning prompt
Look for the plan generation template in simulation_server/
You will edit this in Exercise 5
...
What to do
Read Bernard's scratch file. Copy the original text into your answer template — you will need it for comparison.

Edit Bernard's scratch file to make him more confrontational. First attempt: moderate changes (e.g., "Bernard is often impatient and critical of sloppy work"). Save and run the simulation for 30 minutes of in-game time.

Inspect the conversation logs. Find conversations between Bernard and the baristas. Copy 2–3 representative exchanges into your answer template.

Edit again. Make Bernard more extreme (e.g., "Bernard is rude, dismissive, and frequently berates his employees"). Run another 30 minutes. Compare the conversations.

Answer the questions in the answer template.
What to look for
...
What to do
Reset all scratch files to their originals (undo Exercise 1 changes).

Run the simulation with Model A (e.g., GPT-4o-mini) for 30 minutes of in-game time. Save the logs to a clearly named folder (e.g., logs_model_a/).

Run the same simulation with Model B (e.g., Llama 3 via Ollama) for 30 minutes. Save logs to logs_model_b/.

Compare using the analysis starter script:

python course/analysis_starter.py --log1 logs_model_a/ --log2 logs_model_b/

Read the conversation logs qualitatively. Note differences in tone, vocabulary, and interaction patterns.

Count JSON validation failures. Check the simulation server logs for parsing errors. Record the count for each model.
...
Time estimate: 45–60 minutes.
What to do
Choose one model (whichever worked better in Exercise 2). Use this model for the remaining exercises.

Add one sentence to Dolores's scratch file. We suggest:
...
Run a second perturbation. Restore the original scratch file, then change 2–3 adjectives in Dolores's personality traits (e.g., "warm" → "guarded", "detail-oriented" → "easily distracted"). Run 30 minutes. Save logs as logs_perturbation_traits/.

Compare both perturbations to the baseline (your Model A run from Exercise 2 serves as baseline). Use the analysis script to plot valence trajectories and compute effect sizes.
...
What to do
Restore all scratch files to their originals.

Enable the negativity bias for Dolores. Follow the instructions in the README to activate:
...
Run the simulation for at least 30 minutes of in-game time. Save logs as logs_negbias/. If you can, run longer (4–6 hours in-game) — the effects become more pronounced over time.

Quantitative analysis. Using the analysis script:
...
Thought description comparison. Find 2–3 of Dolores's thought descriptions for negative events and compare their length and content to Maeve's for comparable events. Are Dolores's descriptions longer? Do they contain sensory language (e.g., physical descriptions of how interactions felt)?

Check for memory intrusion. If your simulation ran long enough to include off-duty hours (evening, at home), inspect Dolores's reflections from that period. Do work memories intrude — references to Bernard, the café, or workplace events? Compare with Maeve's evening reflections. If you find examples, paste them into your answer template.

Compare to Exercise 3. Does the architectural change (negativity bias) produce a larger or smaller effect than the single-sentence perturbation?

(Optional but recommended) Run the simulation twice more to check consistency.
...
Time estimate: 45–60 minutes.
What to do
Inspect daily plans. From any of your previous runs, find the daily plans generated for each agent at the start of each simulated day. Look at the log entries that show the agent's intended schedule.

Identify problems. Common issues:
...
Write planning rules. Create a set of 3–5 natural language rules that should improve plan quality. For example:
...
Inject your rules. Find the planning prompt template in the codebase and add your rules to it. You are editing natural language, not code.

Run the simulation for 30 minutes with your improved planning prompt. Compare the generated plans to the originals.

Assess downstream effects. Do agents with better plans have more social interactions? More varied activities? Different valence trajectories?
