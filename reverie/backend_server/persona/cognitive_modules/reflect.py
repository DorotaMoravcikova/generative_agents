...
def run_reflect(persona):
    ...
    for focal_pt, nodes in retrieved.items():
        ...
        for thought, evidence in thoughts.items():
            ...
            expiration = persona.scratch.curr_time + \
                datetime.timedelta(days=30)
            s, p, o = generate_action_event_triple(thought, persona)
            keywords = set([s, p, o])
            thought_poignancy = generate_poig_score(
                persona, "thought", thought)
...
def reflect(persona):
    ...
    if persona.scratch.chatting_end_time:
        ...
        if persona.scratch.curr_time + datetime.timedelta(0, 10) == persona.scratch.chatting_end_time:
            ...
            evidence = [persona.a_mem.get_last_chat(
                persona.scratch.chatting_with).node_id]
            ...
            thought_embedding_pair = (
                planning_thought, get_embedding(planning_thought))
            ...
            keywords = set([s, p, o])
            thought_poignancy = generate_poig_score(
                persona, "thought", memo_thought)
            thought_embedding_pair = (
                memo_thought, get_embedding(memo_thought))
