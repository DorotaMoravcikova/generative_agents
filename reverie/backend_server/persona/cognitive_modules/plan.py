...
def generate_decide_to_talk(init_persona, target_persona, retrieved):
    x = run_gpt_prompt_decide_to_talk(
        init_persona, target_persona, retrieved)[0]
    if debug:
...
def generate_new_decomp_schedule(persona, inserted_act, inserted_act_dur,  start_hour, end_hour):
    ...
    ) + " (on the way to " + truncated_act_dur[-1][0].split("(")[-1][:-1] + ")"
...
def _create_react(persona, inserted_act, inserted_act_dur,
    ...
    if (p.scratch.f_daily_schedule_hourly_org[p.scratch.get_f_daily_schedule_hourly_org_index()][1] >= 120):
        end_hour = start_hour + \
            p.scratch.f_daily_schedule_hourly_org[p.scratch.get_f_daily_schedule_hourly_org_index(
            )][1]/60
...
def _chat_react(maze, persona, focused_event, reaction_mode, personas):
    ...
    if curr_time.second != 0:
        temp_curr_time = curr_time + \
            datetime.timedelta(seconds=60 - curr_time.second)
        chatting_end_time = temp_curr_time + \
            datetime.timedelta(minutes=inserted_act_dur)
    else:
        chatting_end_time = curr_time + \
            datetime.timedelta(minutes=inserted_act_dur)
